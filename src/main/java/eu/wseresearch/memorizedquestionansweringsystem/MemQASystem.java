...
import eu.wseresearch.memorizedquestionansweringsystem.exception.DatasetNotExist;
import eu.wseresearch.memorizedquestionansweringsystem.exception.QuestionNotExist;
import eu.wseresearch.memorizedquestionansweringsystem.messages.response.memanswer.MemAnswer;
import eu.wseresearch.memorizedquestionansweringsystem.messages.response.memanswer.MemAnswerQuery;
...
@Component
public class MemQASystem {
    private static final Logger LOGGER = LoggerFactory.getLogger(MemQASystem.class);
...
//        knowledgebases.add(this.knowledgebase);
        ...
        List<MemAnswerQuery> queries = createAnswerQueries(question, language, numberOfResultsItems, dataset);
        ...
        return memAnswer.toJsonObject();
    }

    public List<MemAnswerQuery> createAnswerQueries(
            String question,
            String language,
            Integer numberOfResultsItems,
            String dataset
        ...
        List<MemAnswerQuery> incorrectAnswers = this.createIncorrectAnswers(datasetResource, question, language, numberOfResultsItems);
        for (MemAnswerQuery incorrectAnswer : incorrectAnswers) {
    ...
    public MemAnswerQuery createCorrectAnswerQuery(
            Resource questionResource
    ...
    public List<MemAnswerQuery> createIncorrectAnswers(
            Resource datasetResource,
            String questionText,
            String language,
            Integer numberOfResultsItems
    ) throws IOException, SparqlQueryFailed {
        ArrayList<MemAnswerQuery> memAnswerQueries = new ArrayList<>();
        ...
        for (String wrongAnswerSparql : wrongAnswers) {
            ...
            memAnswerQueries.add(new MemAnswerQuery(
                    wrongAnswerSparql,
                    false,
                    confidence,
                    this.knowledgebase,
